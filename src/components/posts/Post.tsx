  ...
  return (
    <article className="group/post space-y-3 rounded-2xl bg-card p-5 shadow-sm">
      ...
      {!!post.attachments.length && (
        <MedaPreviews attachments={post.attachments}/>
      )}
      ...
      {showComment && <Commments post={post}/>}
    </article>
  );
}

interface MedaPreviewsProps {
  attachments: Media[];
}

function MedaPreviews({ attachments }: MedaPreviewsProps) {
  return (
    <div
      className={cn(
        "flex flex-col gap-3",
...
interface MedaPreviewProps {
  media: Media;
}

function MediaPreview({ media }: MedaPreviewProps) {
  if (media.type === "IMAGE") {
    return (
      <Image
        src={media.url}
